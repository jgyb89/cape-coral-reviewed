...
export default async function DirectoryListingPage({
  ...
  const averageRating = reviewCount > 0 ? (reviewNodes.reduce((acc, curr) => acc + (Number.parseFloat(curr.reviewFields?.starRating) || 0), 0) / reviewCount).toFixed(1) : null;
  const initialIsFavorite = currentUser?.userData?.favoriteListings?.nodes?.some(n => n.databaseId === listing.databaseId) || false;
  const featuredImage = formatImageUrl(listing.featuredImage?.node?.sourceUrl);
  const galleryImages = listing.attachedMedia?.nodes?.map(node => formatImageUrl(node.sourceUrl)) || [];
  const cleanContent = DOMPurify.sanitize(listing.content || "", {
    ALLOWED_TAGS: ["p", "br", "b", "i", "em", "strong", "a", "ul", "ol", "li", "h3", "h4"],
    ALLOWED_ATTR: ["href", "target", "rel"]
  });
  const socialLinks = listingdata.socialUrl ? listingdata.socialUrl.split(",").map(s => s.trim()).filter(s => s !== "") : [];
  const jsonLd = {
    ...
    url: listingdata.websiteUrl || `${BASE_URL}/listing/${slug}`,
    ...
    openingHours: [listingdata.hoursMonday && `Mo ${listingdata.hoursMonday}`, listingdata.hoursTuesday && `Tu ${listingdata.hoursTuesday}`, listingdata.hoursWednesday && `We ${listingdata.hoursWednesday}`, listingdata.hoursThursday && `Th ${listingdata.hoursThursday}`, listingdata.hoursFriday && `Fr ${listingdata.hoursFriday}`, listingdata.hoursSaturday && `Sa ${listingdata.hoursSaturday}`, listingdata.hoursSunday && `Su ${listingdata.hoursSunday}`].filter(Boolean)
  };
  const dayLabels = t.days || {};
  const hours = [{
    day: dayLabels.monday || "Monday",
    time: listingdata.hoursMonday
  }, {
    day: dayLabels.tuesday || "Tuesday",
    time: listingdata.hoursTuesday
  }, {
    day: dayLabels.wednesday || "Wednesday",
    time: listingdata.hoursWednesday
  }, {
    day: dayLabels.thursday || "Thursday",
    time: listingdata.hoursThursday
  }, {
    day: dayLabels.friday || "Friday",
    time: listingdata.hoursFriday
  }, {
    day: dayLabels.saturday || "Saturday",
    time: listingdata.hoursSaturday
  }, {
    day: dayLabels.sunday || "Sunday",
    time: listingdata.hoursSunday
  }];

  // Process Video and Social Links
  const videoEmbedUrl = getEmbedUrl(listingdata.videoUrl);
  return <div className="listing-layout">
      <Script id="listing-jsonld" type="application/ld+json" dangerouslySetInnerHTML={{
      ...
      <main className="listing-main">
        ...
        <header className="listing-header">
          ...
          <div className="listing-header__meta">
            ...
            <div className="listing-header__categories">
              {listing.directoryTypes?.nodes?.map(cat => <span key={cat.slug} className="listing-category-tag">
                  {cat.name}
                </span>)}
            </div>
          </div>
        ...
        <section id="reviews" className="listing-card">
          <div className="reviews-section-header">
            ...
            <ReviewActionManager listingId={listing.databaseId} listingTitle={listing.title} currentUser={currentUser} dict={dict} />
          </div>
      ...
      <aside className="listing-sidebar">
        <ClaimListing listingId={listing.databaseId} listingTitle={listing.title} isClaimed={listing.listingdata?.isClaimed === "true"} dict={dict} />
