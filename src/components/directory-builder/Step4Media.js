...
const Step4Media = ({ formData, updateFormData, nextStep, prevStep }) => {
  ...
  const processFiles = (incomingFiles, field) => {
    ...
    const validFiles = incomingFiles.filter(file => {
      if (!allowedTypes.includes(file.type)) {
        currentErrors.push(`"${file.name}" is not a valid format (JPEG, PNG, WEBP only).`);
        return false;
      }
    ...
    if (field === 'featuredImage') {
      ...
      if (newGallery.length > 10) {
        currentErrors.push('Maximum 10 images allowed. Excess images were removed.');
  ...
  const removeGalleryImage = (indexToRemove) => {
    const newGallery = formData.gallery.filter((_, index) => index !== indexToRemove);
    updateFormData({ gallery: newGallery });
  ...
  return (
    <div className={styles['step-form']}>
      ...
      <div className={styles['step-form__group']}>
        ...
        {!formData.featuredImage ? (
          ...
          >
            ...
            <p style={{ margin: '0 0 1rem 0', color: '#475569' }}>Drag & drop your featured image here, or</p>
            <input
              type="file"
              id="featuredImage"
              ...
              style={{ display: 'none' }}
            />
            <label htmlFor="featuredImage" style={{ cursor: 'pointer', backgroundColor: '#fff', border: '1px solid #cbd5e1', padding: '0.5rem 1rem', borderRadius: '6px', color: '#1e293b', fontWeight: 600, display: 'inline-block' }}>
              Browse Files
            </label>
          </div>
        ) : (
          <div style={{ position: 'relative', width: '200px', height: '140px', borderRadius: '8px', overflow: 'hidden', border: '1px solid #e2e8f0' }}>
            <img 
            ...
            <button 
              ...
              title="Remove image"
              style={{ position: 'absolute', top: '0.5rem', right: '0.5rem', background: '#ef4444', color: 'white', border: 'none', borderRadius: '50%', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 2px 4px rgba(0,0,0,0.2)' }}
            >
        ...
        {fileErrors.featured && (
          <div style={{ color: '#ef4444', fontSize: '0.9rem', marginTop: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.25rem', fontWeight: 500 }}>
      ...
      {/* GALLERY IMAGES */}
      <div className={styles['step-form__group']}>
        <label className={styles['step-form__label']}>Gallery Images (Max 10)</label>
          ...
          {formData.gallery && formData.gallery.length > 0 ? (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem' }}>
              {formData.gallery.map((file, index) => (
                <div key={index} style={{ position: 'relative', width: '100px', height: '100px', borderRadius: '8px', overflow: 'hidden', border: '1px solid #e2e8f0', backgroundColor: '#fff' }}>
                  <img 
                  ...
                  <button 
                    ...
                    title="Remove image"
                    style={{ position: 'absolute', top: '0.25rem', right: '0.25rem', background: '#ef4444', color: 'white', border: 'none', borderRadius: '50%', width: '22px', height: '22px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 2px 4px rgba(0,0,0,0.2)' }}
                  >
              ...
              {formData.gallery.length < 10 && (
                <div style={{ width: '100px', height: '100px', borderRadius: '8px', border: '1px solid #cbd5e1', backgroundColor: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <input
                    type="file"
                    id="gallery"
                    accept="image/jpeg, image/png, image/webp"
                    ...
                    style={{ display: 'none' }}
                  />
                  <label htmlFor="gallery" style={{ cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', color: '#64748b' }}>
            ...
            <div style={{ textAlign: 'center', padding: '1rem 0' }}>
              <span className="material-symbols-outlined" style={{ fontSize: '2.5rem', color: '#94a3b8', marginBottom: '0.5rem' }}>collections</span>
              <p style={{ margin: '0 0 1rem 0', color: '#475569' }}>Drag & drop your gallery images here</p>
              <input
                type="file"
                id="gallery"
                accept="image/jpeg, image/png, image/webp"
                ...
                style={{ display: 'none' }}
              />
              <label htmlFor="gallery" style={{ cursor: 'pointer', backgroundColor: '#fff', border: '1px solid #cbd5e1', padding: '0.5rem 1rem', borderRadius: '6px', color: '#1e293b', fontWeight: 600, display: 'inline-block' }}>
                Browse Files
              </label>
            </div>
          )}
      ...
      <div className={styles['step-form__group']}>
        ...
        />
        {errors.videoUrl && <span className={styles['step-form__error-message']} style={{ color: '#ef4444', fontSize: '0.9rem', marginTop: '0.25rem', display: 'block' }}>{errors.videoUrl}</span>}
      </div>
      ...
      <div className={wizardStyles['wizard__actions']}>
        ...
        <button 
          ...
          style={{ opacity: (fileErrors.featured || fileErrors.gallery) ? 0.5 : 1, cursor: (fileErrors.featured || fileErrors.gallery) ? 'not-allowed' : 'pointer' }}
        >
          Next
        </button>
      </div>
